...
void TrajectoryGenerator::checkHalfWay()
{
            ...
            && _core->checkHalfWayObstacle_BrutalForce(_odom.header.stamp.toSec()))
    {
        ...
        for (size_t idx = vaild_id; idx < _waypoints.size(); idx += NUM_DIM)
        {
            path.poses[idx - vaild_id].pose.position.x = _waypoints[idx + X];
            path.poses[idx - vaild_id].pose.position.y = _waypoints[idx + Y];
            path.poses[idx - vaild_id].pose.position.z = _waypoints[idx + Z];
        }
...
void TrajectoryGenerator::rcv3DLaserScan(const sensor_msgs::PointCloud2 & cloud)
{
    ...
    if (!retRaysFromCoud2Odom(cloud, laser_odom, rays, 
